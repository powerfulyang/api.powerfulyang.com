...
export class BucketService {
    ...
    private async fetchAllBuckets() {
        ...
        const listBucketAcl = (await Promise.all(
            listBucketAclPromises,
        )) as any;
        ...
        const listBucketCors = (await Promise.all(
            listBucketCorsPromises,
        )) as any;
        ...
        const listBucketReferer = (await Promise.all(
            listBucketRefererPromises,
        )) as any;
        const arr = list.map((bucket, index) => ({
            ...
            referer: produce(
                ...
                (draft: { DomainList: { Domains: any[] } }) => {
                    draft.DomainList.Domains = draft.DomainList.Domains.map(
                        (domain) => Object.values(domain).join(''),
                    );
                },
            ),
        }));
