...
import type { RedisValue } from '@/type/RedisValue.mjs';
...
export class CacheService {
  ...
  hSet<T = any>(hash: string, hKey: string | number, val: T) {
    ...
    return promisify(this.redisClient.hset).call(this.redisClient, hash, hKey, value);
  }

  async hGet<T = any>(hash: string, hKey: string | number): Promise<T> {
    const val = await promisify(this.redisClient.hget).call(this.redisClient, hash, hKey);
    return JSON.parse(val);
  }

  hMSet(hash: string, map: Record<string, RedisValue>) {
    return promisify(this.redisClient.hmset).call(this.redisClient, hash, map);
  }

  sAdd(sKey: string, arr: RedisValue[] | RedisValue) {
    return promisify(this.redisClient.sadd).call(this.redisClient, sKey, arr);
  }
  ...
  del(key: string) {
    return promisify(this.redisClient.del).call(this.redisClient, key);
  }
}
