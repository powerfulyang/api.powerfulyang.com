...
export class UploadStaticService {
    constructor(@InjectRepository(StaticResource) private staticDao: Repository<StaticResource>) {}
    ...
    async persistent(sha1: string) {
        ...
        const originBuffer = await readFileSync(join(originPath, staticResource.path.origin));
        const resizeBuffer = await sharp(originBuffer).resize(300).toBuffer();
        ...
        const bucket = staticResource.bucket;
        await this.putCosObject(bucket, originBuffer, staticResource.path.origin);
        await this.putCosObject(bucket, resizeBuffer, staticResource.path.resize);
        await this.putCosObject(bucket, webpBuffer, staticResource.path.webp);
    }
