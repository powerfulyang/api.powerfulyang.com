...
export class GitHubEventsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
    ...
    const githubEvent: EventType = request.headers['x-github-event'];

    return GitHubEventsGuard._checkValid(signature, githubEvent, restrictedEvents, request.body);
  }
  ...
  private static _checkSignature(signature: string, payload: Payload<WebhookPayload>): boolean {
    ...
    if (signature !== digest) {
      throw new ForbiddenException(`Request body digest (${digest}) does not match ${signature}`);
    }
